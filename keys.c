...
/*   Updated: 2023/05/02 12:02:06 by abazerou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	down_key(t_vars *v)
{
	...
	while (v->map[v->i])
	{
		...
		while (v->map[v->i][v->j])
		{
			if (v->map[v->i][v->j] == 'P' && (v->map[v->i + 1][v->j] == 'C'
			|| v->map[v->i + 1][v->j] == '0' || v->map[v->i + 1][v->j] == 'E'))
			{
...
void	left_key(t_vars *v)
{
	...
	while (v->map[v->i])
	{
		...
		while (v->map[v->i][v->j])
		{
			if (v->map[v->i][v->j] == 'P' && (v->map[v->i][v->j - 1] == 'C'
			|| v->map[v->i][v->j - 1] == '0' || v->map[v->i][v->j - 1] == 'E'))
			{
